...
a unified, declarative configuration for all machines and user environments.
...
system configuration (NixOS) and user configuration (home-manager)
are both declared here — no separate dotfiles manager.

different machine configurations are defined
in [`flake.nix`](./flake.nix).
to rebuild the configuration for a machine, run from this directory:

```nu
sudo nixos-rebuild switch --flake .#green
```

for the standalone macOS home-manager config (eve):
...
user-level configuration (packages, dotfiles, shell) is managed by
home-manager and lives under `home/`.

generated hardware configurations are defined by
`<machine-name>-hardware-configuration.nix`.

machine specific configurations are defined in `modules/<machine-name>.nix`.
...
to build the rescue disk ISO, run from this directory:
